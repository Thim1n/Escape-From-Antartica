...
class RoundEnemy extends Enemy {
    ...
    checkCollision(player) {
        const distX = Math.abs(player.x + player.width / 2 - (this.x + this.radius));
        const distY = Math.abs(player.y + player.height / 2 - (this.y + this.radius));
...
function createEnemies(canvas) {
    ...
    enemies.push(new MovingEnemy(710, height - 650, 600, 820, height - 460, height - 460, 2.5));

    // Ajout de l'ennemi rond avec mouvement gauche-droite
    enemies.push(new RoundEnemy(2500, 475, 80, "../assets/sprite/scie.png", 14, 1700, 3400));  // Déplacement entre x = 2000 et x = 2700
    ...
    spikeGroups.forEach(group => {
        for (let i = 0; i < group.count; i++) {
            enemies.push(new SpikeEnemy(
                group.startX + (i * group.spacing),
                group.y
            ));
        }
    });
