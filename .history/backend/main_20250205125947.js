...
app.get("/leaderboard", async (req, res) => {
  try {
    // Récupérer les utilisateurs avec leurs meilleurs scores et leur meilleur temps
    const leaderboard = await Game.findAll({
      attributes: [
        "userId",
        [sequelize.fn("MIN", sequelize.col("time")), "minTime"], // On prend le temps minimum pour chaque utilisateur
        [sequelize.fn("MAX", sequelize.col("score")), "maxScore"], // On prend le score maximum pour chaque utilisateur
      ],
      include: [
        {
          model: User,
    ...
    // Retourner la liste des scores avec les informations correctes
